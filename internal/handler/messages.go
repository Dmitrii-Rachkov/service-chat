...
// MessageDelete - удалить сообщение от лица пользователя
...
// @Accept json
// @Produce json
// @Param input body dto.MessageDelete true "message info"
// @Success 200 {object} Response
// @Failure 400,404,405 {object} Response
// @Failure 500 {object} Response
// @Failure default {object} Response
// @Router /messages/delete [delete]
func (h *Handler) MessageDelete(log *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Логируем наш запрос
		const op = "handler.MessageDelete"
		log = log.With(
			slog.String("op", op),
			slog.String("request_id", middleware.GetReqID(r.Context())),
		)
		...
		// Отправляем валидную структуру на слой сервиса
		msgDel, errMsg := h.services.Message.DeleteMessage(req, idCtx)
		if errMsg != nil {
			log.Error("failed to delete message", logger.Err(errMsg))
			render.JSON(w, r, Error(fmt.Sprintf("Failed to delete message: %s", errMsg)))
			return
		}

		// Если ошибок нет отправляем успешный ответ
		log.Info("Messages delete successfully", "Messages", msgDel)
		render.JSON(w, r, Response{
			Status:     StatusOK,
